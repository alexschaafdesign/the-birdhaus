...
// separate from `node scripts/migrate.mjs`, which always targets dev (it loads
// .env.local, and the shell's DATABASE_URL is dev by convention — see
// docs/db-safety.md).
//
...
// Variables > Production > DATABASE_URL > reveal. `vercel env pull` won't work
// here — Vercel redacts sensitive vars on pull, so it comes back empty.
//
// Run: node scripts/migrate-prod.mjs   (or npm run db:migrate:prod)
...
// Guard against a mislabeled file: the dev branch is ep-calm-bonus-... (see
// docs/db-safety.md). If .env.prod.local somehow points there, this isn't prod.
if (host.startsWith('ep-calm-bonus-')) {
  console.error(`\n❌ ${PROD_ENV_FILE} points at the DEV host (${host}), not prod. Aborting.\n`);
  process.exit(1);
}

const sql = postgres(url, { max: 1, ssl: sslOptionFor(url) });
let dbName, dbUser;
try {
...
// Delegate to the real runner in a child process so DATABASE_URL is scoped to
// this one invocation, never touching the parent shell's env (which stays
// pointed at dev per .env.local/.envrc). migrate.mjs calls loadEnvFile('.env.local'),
// but Node's loadEnvFile doesn't override an already-set env var, so the prod
// URL we pass here wins.
