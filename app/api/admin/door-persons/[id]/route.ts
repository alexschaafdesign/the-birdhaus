...
export async function PATCH(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const doorPersonId = parseId(id);
  if (doorPersonId === null) {
...
export async function DELETE(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  // Nothing references door_persons by FK (settlements keep a free-text name),
  // so a delete just drops the profile.
  await sql`delete from door_persons where id = ${doorPersonId}`;
  return NextResponse.json({ ok: true });
}
