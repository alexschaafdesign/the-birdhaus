...
// Three-step upload, same shape as the Song Club track form: ask the API for
// a presigned URL, PUT the audio straight to R2 (XHR, for upload progress —
// the file never touches Vercel), then register the key as a version of this
// song.
export default function BandVersionUpload({
  ...
  if (!open) {
    return (
      <button
        ...
        className="text-sm text-[#E8E0D0]/50 underline-offset-2 transition hover:text-[#E8E0D0] hover:underline"
      >
        + Upload a version
      </button>
    );
  ...
  return (
    <form
      onSubmit={submit}
      className="space-y-4 rounded-lg border border-[#E8E0D0]/15 bg-[#E8E0D0]/[0.03] p-4"
    >
      <div>
      ...
      <div>
        ...
        <input
          type="text"
          id="version-label"
          placeholder={file ? file.name.replace(/\.[^.]+$/, '') : 'voice memo, demo v2…'}
          value={label}
